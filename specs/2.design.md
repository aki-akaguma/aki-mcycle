...
- **Responsibility:** This module defines the entire command-line interface using the `flood_tide` crate. The `conf::parse::CmdOptConf` struct defines all valid arguments and flags.
- **Design:** It uses `flood-tide-gen` to automatically generate the command-line parser and help text from definitions. This ensures consistency between the CLI behavior and the documented help messages.
- **Maps to Acceptance Criteria:**
    - **AC6, AC7:** The `-H`/`--help` and `-V`/`--version` flags are defined and handled here.
